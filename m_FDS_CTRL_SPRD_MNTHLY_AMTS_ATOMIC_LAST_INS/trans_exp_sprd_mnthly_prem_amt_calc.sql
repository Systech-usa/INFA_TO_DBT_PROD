...
trans_exp_sprd_mnthly_prem_amt_calc as (
    select
        ...
        IFF(
            sq.v_chk_mnth_part_fm = 1 OR sq.v_chk_mnth_part_to = 1,
            IFF(
                sq.out_trl_sprd_ind = 0,
                ROUND(sq.v_days_in_mnth * sq.daily_amt, 2),
                IFF(
                    sq.v_mnth_yr_part_last_day <= sq.v_mnth_yr_part_trl_sprd_2yr_last_day,
    ...
    left join trans_exp_eff_to eff
        on sq.eff_fm_tistmp = eff.EFF_FM_TISTMP 
        and sq.eff_to_tistmp = eff.in_EFF_TO_TISTMP
)
