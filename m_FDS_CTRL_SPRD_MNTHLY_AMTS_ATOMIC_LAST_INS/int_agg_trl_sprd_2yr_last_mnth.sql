...
int_agg_trl_sprd_2yr_last_mnth as (
    select 
        ...
        -- Input / Input/Output Fields
        MAX(MONTHLY_DATE) as MONTHLY_DATE,
        MAX(AGMT_ANCHR_ID) as AGMT_ANCHR_ID1,
        MAX(BASE_AMT) as BASE_AMT1,
        MAX(DAILY_AMT) as DAILY_AMT1,
        MAX(TRL_SPRD_IND) as TRL_SPRD_IND,
        MAX(MNTH_YR_PART_TRL_SPRD_2YR_LAST_DAY) as MNTH_YR_PART_TRL_SPRD_2YR_LAST_DAY1,
        MAX(TRL_SPRD_2YR_BASE_AMT) as TRL_SPRD_2YR_BASE_AMT1,
        MAX(TRL_SPRD_2YR_LAST_MNTH_REM_DAYS) as TRL_SPRD_2YR_LAST_MNTH_REM_DAYS1,
        MAX(TRL_SPRD_REM_TERM_DLY_AMT) as TRL_SPRD_REM_TERM_DLY_AMT1,
        MAX(PREM_AMT) as PREM_AMT1,
        MAX(EFF_MD_YR_MO) as EFF_MD_YR_MO1,
        MAX(POP_INFO_ID) as POP_INFO_ID1,
        MAX(BATCH_ID) as BATCH_ID1,
        MAX(MNTH_YR_PART_EFF_TO) as MNTH_YR_PART_EFF_TO1,
        MAX(MNTH_YR_PART_LAST_DAY) as MNTH_YR_PART_LAST_DAY1,
        MAX(COVG_CMPNT_AGMT_ANCHR_ID) as COVG_CMPNT_AGMT_ANCHR_ID1,
        MAX(out_COMM_AMT) as out_COMM_AMT1,
        MAX(TRL_SPRD_2YR_COMM_AMT) as TRL_SPRD_2YR_COMM_AMT1,
        MAX(TRL_SPRD_REM_TERM_COMM_DLY_AMT) as TRL_SPRD_REM_TERM_COMM_DLY_AMT1,
        MAX(out_DER_COMM_AMT) as out_DER_COMM_AMT,
        MAX(out_TRAN_PREM_AMT) as out_TRAN_PREM_AMT,
        MAX(TRAN_COMM_AMT) as TRAN_COMM_AMT1,
        MAX(TRL_SPRD_2YR_TRAN_PREM_AMT) as TRL_SPRD_2YR_TRAN_PREM_AMT1,
        MAX(TRL_SPRD_REM_TERM_TRAN_PREM_DLY_AMT) as TRL_SPRD_REM_TERM_TRAN_PREM_DLY_AMT1,
        MAX(out_DER_TRAN_PREM_AMT) as out_DER_TRAN_PREM_AMT,
        MAX(out_TRL_SPRD_2YR_TRAN_COMM_AMT) as out_TRL_SPRD_2YR_TRAN_COMM_AMT1,
        MAX(TRL_SPRD_REM_TERM_TRAN_COMM_DLY_AMT) as TRL_SPRD_REM_TERM_TRAN_COMM_DLY_AMT1,
        ...
        -- Explicitly defined output aggregated fields
