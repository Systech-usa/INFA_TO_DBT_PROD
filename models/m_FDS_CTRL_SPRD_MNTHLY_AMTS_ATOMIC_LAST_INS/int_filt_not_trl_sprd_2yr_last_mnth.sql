...
with exp_sprd_2yr_mnthly_prem_amt_calc as (
    select * from {{ ref('trans_exp_sprd_2yr_mnthly_prem_amt_calc') }}
),

sq_ctrl_sprd_mnthly_amts_temp as (
...
joined_data as (
    ...
    from exp_sprd_2yr_mnthly_prem_amt_calc exp
    left join sq_ctrl_sprd_mnthly_amts_temp sq
...
int_filt_not_trl_sprd_2yr_last_mnth as (
    select *
    from joined_data
    where out_trl_sprd_ind = 0 or ( out_trl_sprd_ind = 1 and mnth_yr_part_last_day <> out_mnth_yr_part_trl_sprd_2yr_last_day )
)
